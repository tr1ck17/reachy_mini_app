...
with expressive robot behaviors, live dashboard, and persistent session memory.
...
5\. LLM generates a facilitation response (Claude API or Ollama fallback)

6\. edge-tts speaks the response aloud

7\. Reachy Mini moves expressively based on detected mood

8\. Dashboard updates with transcript and artifacts in real time
...
\*\*`reachy\_chat.py`\*\* — Main entry point. Handles the full conversation loop:

\- Session mode selection (continue vs fresh) via terminal prompt or `REACHY\_SESSION\_MODE` env var
...
\- Response parsing — extracts MOOD: tag and ARTIFACT\_\*: tags
...
\*\*`cps\_manager.py`\*\* — CPS stage management:

\- `STAGES` — ordered list: `\["clarify", "ideate", "develop", "implement"]`

\- `ADVANCE\_KEYWORDS` — intentionally strict list of phrases that trigger stage advance

&#x20; (avoids false positives from normal conversation words like "ready" or "advance")

\- `load\_stage()` — reads knowledge base `.md` file for a given stage

\- `build\_system\_prompt()` — injects stage knowledge into the base system prompt

\- `check\_for\_advance()` — checks if user's message contains an explicit advance phrase

\- `next\_stage()` — returns next stage or None if at final stage



\*\*`memory\_manager.py`\*\* — Session persistence:

\- Rolling memory: last 5 sessions stored in `memory.json`

\- Stage state: current CPS stage stored in `stage\_state.json`

\- Session ID: unique ID per CPS problem stored in `session\_id.json`

\- Transcript export: all sessions for one problem append to one `sessions/session\_<id>.md` file

&#x20; — first session creates the file, continuations append with a `## Session continued` marker

\- `build\_history\_context()` — flattens past sessions into LLM message list with timestamps



\*\*`dashboard\_state.py`\*\* — Shared in-memory state between `reachy\_chat.py` and `launcher.py`:
...
\- `/api/launch` — launches `reachy\_chat.py` in a new terminal via subprocess

\- `/api/clear-history` — deletes memory.json, stage\_state.json, session\_id.json, all .md transcripts
...
\- \*\*Launcher state\*\*: session info card, Continue/Start New/Clear History buttons
...
\- `stage\_state.json` — current CPS stage

\- `session\_id.json` — current problem's session ID

\- `sessions/` — transcript `.md` files, one per CPS problem

\- `reachy.log` — detailed application log

\- `input\_\*.wav`, `tts\_\*.wav` — temporary audio files (deleted after each turn)
...
\- User says explicit phrase → `check\_for\_advance()` catches it → stage advances
...
\- \*\*MIN\_AUDIO\_VOLUME = 0.0005\*\* — lowered to avoid dropping out at normal mic distance

\- Reachy's TTS playback can bleed into the next recording if timing is tight — add `time.sleep(0.5)`

&#x20; at the start of `record\_audio()` if this occurs



\### Artifact Extraction

\- LLM tags key artifacts in responses with `ARTIFACT\_\*:` prefixes

\- `parse\_response()` strips these tags from spoken text and routes them to `dashboard\_state.py`

\- Claude follows these tags reliably; Ollama mostly ignores them

\- Artifacts accumulate across the session — dashboard shows full running list



\### Transcript Merging

\- Each CPS problem gets a unique `session\_id` stored in `session\_id.json`

\- Transcript file is `sessions/session\_<id>.md`
...
\- This gives a complete record of each problem's full journey across multiple days/sessions
...
If recording starts immediately after Reachy finishes speaking, Whisper may transcribe

the tail end of Reachy's TTS audio as user input, producing garbled text.

\*\*Fix: add `time.sleep(0.5)` at start of `record\_audio()` if this occurs.\*\*
...
Ollama ignores the `ARTIFACT\_\*:` tags in the system prompt.
...
\- \[ ] Web dashboard artifact export (download session artifacts as PDF/doc)
...
$env:ANTHROPIC\_API\_KEY="your-key"     # enables Claude API (strongly recommended)

$env:REACHY\_SESSION\_MODE="continue"   # set by launcher, skip terminal prompt

$env:REACHY\_SESSION\_MODE="new"        # set by launcher for fresh sessions
...
$env:ANTHROPIC\_API\_KEY="your-key"
